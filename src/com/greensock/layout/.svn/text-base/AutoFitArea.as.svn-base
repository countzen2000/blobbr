		...
		public function update(event:Event=null):void {
			...
			while (item) {
				...
				if (scaleMode != ScaleMode.NONE) {
					...
					bounds = (item.calculateVisible) ? item.getVisibleBounds(target) : target.getBounds(target);
				...
				bounds = (item.calculateVisible) ? item.getVisibleBounds(_parent) : target.getBounds(_parent);
...
internal class AutoFitItem {
	...
	private static var _bd:BitmapData = new BitmapData(2880, 2880, true, 0x00FFFFFF);
	private static var _rect:Rectangle = new Rectangle(0, 0, 2880, 2880);
	...
	public function getVisibleBounds(targetCoordinateSpace:DisplayObject):Rectangle {
		...
		var offset:Rectangle = this.target.getBounds(targetCoordinateSpace);
		var m:Matrix = (targetCoordinateSpace == this.target) ? _matrix : this.target.transform.matrix;
		m.tx -= offset.x;
		m.ty -= offset.y;
		_bd.draw(this.target, m, null, "normal", _rect, false);
		var bounds:Rectangle = _bd.getColorBoundsRect(0xFF000000, 0x00000000, false);
		bounds.x += offset.x;
		bounds.y += offset.y;
		if (targetCoordinateSpace == this.target) {
			_bounds = bounds;
		}
		return bounds;
	}
	...
	public function setVisibleWidth(value:Number):void {
		...
		var curWidth:Number = (m.a < 0) ? -m.a * _bounds.width : m.a * _bounds.width;
		curWidth += (m.c < 0) ? -m.c * _bounds.height : m.c * _bounds.height;
	...
	public function setVisibleHeight(value:Number):void {
		...
		var curHeight:Number = (m.b < 0) ? -m.b * _bounds.width : m.b * _bounds.width;
		curHeight += (m.d < 0) ? -m.d * _bounds.height : m.d * _bounds.height;
